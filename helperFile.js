    ...
    if (useragent.isAndroid) {
      if (deepLink.App.app_name === "WhatsApp") {
        ...
        if (substr.includes("'?text='")) {
          ...
          androidLink = deepLink.android_link.replace(slugNumber, number);
        ...
        let androidLink = deepLink.android_link.replace(slugTeam, teamId);
      ...
      if (deepLink.App.app_name === "WhatsApp") {
        ...
        iosLink = deepLink.ios_link.replace(deepLink.event_slug, userName);
        return iosLink;
      }
    } else {
      return link;
...
const pagebuilderThemes = (key, name, opacity = null) => {
  ...
  if (name && name.startsWith("http")) {
    ...
    (name && name.substring(3, 3) === "jpg" && name.substring(0, 4) === "http")
  ) {
